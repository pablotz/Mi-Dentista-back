...
route = flask.Blueprint("paymentmethod_route", __name__,
...
def add(current_user_id):
    ...
    content = ''
    try:
        new_service = PaymentMethodController.addPaymentMethod(
...
def edit(current_user_id):
    ...
    content = ''
    try:
        edit_service = PaymentMethodController.editPayMethod(
            flask.request, current_user_id)
        status = "OK"
...
def FindPaymentMethod(current_user_id):
    ...
    try:
        ...
        if "_id" not in request.json or request.json["_id"] == 0:
            ...
            payment_method = PaymentMethodController.findPayMethod(
                request.json["_id"])
